...
function getTodayDateSaoPaulo(): string {
  ...
  return formatter.format(new Date()); // e.g. "2026-09-14"
}

function getFormattedDateSaoPaulo(date: Date): string {
  const formatter = new Intl.DateTimeFormat("en-CA", {
    timeZone: "America/Sao_Paulo",
    year: "numeric",
    month: "2-digit",
...
export async function performGenerateBackup(
  dbAdmin: any,
  options: { tipo?: "diario" | "manual"; descricao?: string } = {},
) {
  ...
  const dataReferencia = getTodayDateSaoPaulo();
  const tipo = options.tipo || "diario";
  const desc =
    options.descricao ||
    (tipo === "diario"
  ...
  // Retention cleanup: keep the last 30 daily backups
  try {
    const { data: allBks = [] } = await dbAdmin
      .from("backups_sistema")
      .select("id, criado_em")
      .order("criado_em", { ascending: false });

    if (allBks && allBks.length > 30) {
      const idsToDelete = allBks.slice(30).map((b: any) => b.id);
      await dbAdmin.from("backups_sistema").delete().in("id", idsToDelete);
    }
  } catch (_e) {
    // ignore
  }
...
// 1. GENERATE FULL SYSTEM BACKUP (MANUAL OR ON-DEMAND)
export const generateSistemaBackup = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    return await performGenerateBackup(dbAdmin, {
      tipo: data.tipo || "diario",
      descricao: data.descricao,
    });
  });

// 2. CHECK AND ENSURE DAILY AUTOMATIC BACKUP
export const checkAndEnsureDailyBackup = createServerFn({ method: "POST" })
  .middleware([requireDatabaseAuth])
  .handler(async () => {
    const { dbAdmin } = await import("@/integrations/database/client.server");
    const today = getTodayDateSaoPaulo();

    // Check if there is already a backup created today
    const { data: bks } = await dbAdmin
      .from("backups_sistema")
      .select("id, data_referencia, data_layout, criado_em, tipo, descricao")
      .order("criado_em", { ascending: false })
      .limit(1);
    ...
    if (bks && bks.length > 0) {
      const latest = bks[0];
      const latestDate =
        latest.data_referencia || (latest.criado_em ? latest.criado_em.split("T")[0] : "");
      const latestCreatedSp = latest.criado_em
        ? getFormattedDateSaoPaulo(new Date(latest.criado_em))
        : "";

      if (latestDate === today || latestCreatedSp === today) {
        return {
          status: "up_to_date",
          message: "Backup do dia de hoje já está gerado e atualizado",
          backupId: latest.id,
          dataLayout: latest.data_layout,
        };
      }
    }

    // No backup for today -> auto-generate now
    const newBk = await performGenerateBackup(dbAdmin, {
      tipo: "diario",
      descricao: `Backup Diário Automático (${formatDatePtBr(new Date(), true)})`,
    });

    return {
      status: "created",
      message: "Novo backup diário automático gerado com sucesso",
      backupId: newBk.id,
      dataLayout: newBk.data_layout,
    };
  });

// 3. GET SYSTEM BACKUP (LATEST ACTIVE OR AUTO-ENSURED)
export const getSistemaBackup = createServerFn({ method: "GET" })
  ...
  .handler(async () => {
    ...
    if (!bks || bks.length === 0) {
      ...
      return await performGenerateBackup(dbAdmin, { tipo: "diario" });
    }
    ...
    // If latest is not from today, auto-generate fresh today's backup
    if (latestDate !== today && latestCreatedSp !== today) {
      try {
        return await performGenerateBackup(dbAdmin, { tipo: "diario" });
      } catch (_e) {
...
// 4. GET SYSTEM BACKUP BY ID (FOR HISTORICAL SNAPSHOT VIEWING)
export const getSistemaBackupById = createServerFn({ method: "POST" })
  .middleware([requireDatabaseAuth])
  ...
  .handler(async ({ data }) => {
    ...
    const { data: bk, error } = await dbAdmin
      ...
      .eq("id", data.id)
      .single();
    if (error || !bk) throw new Error("Backup do sistema não encontrado");
    return bk;
  });

// 5. GET LIST OF SYSTEM BACKUPS (HISTORY FOR TABLE / SELECTION)
export const getBackupsSistemaList = createServerFn({ method: "GET" })
  .middleware([requireDatabaseAuth])
  .handler(async () => {
    const { dbAdmin } = await import("@/integrations/database/client.server");
    const { data: existing = [] } = await dbAdmin
      .from("backups_sistema")
      .select(
        "id, criado_em, data_layout, data_referencia, descricao, tipo, total_colaboradores, total_ativos, total_inativos, total_sistemas, total_acessos, total_pendencias, total_operacoes",
      )
      .order("criado_em", { ascending: false })
      .limit(30);
    return existing ?? [];
